<?php
	...
	if(!empty($adtitle) || !empty($location) || !empty($road) || !empty($house) || !empty($floor) || !empty($balcony) || !empty($rentprice) || !empty($servicecharge) || !empty($securitydeposit) || !empty($filename) || !empty($availablefrom) || !empty($contact)){
		$sql = "INSERT INTO roommate(title,location,road,house,floor,balcony,rent_price,service_charge,security_deposit,picture, available_from,contact) VALUES('$adtitle','$location','$road','$house','$floor','$balcony','$rentprice','$servicecharge','$securitydeposit','$folder','$availablefrom','$contact')";
